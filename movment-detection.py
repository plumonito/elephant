...
def detect_motion(video_path, movement_threshold=5000, min_contour_area=500, interval=3, debug=False, cut_top=None,
                  cut_bottom=None, cut_right=None, cut_left=None):
    # Open the video file
    cap = cv2.VideoCapture(video_path)

    fps = cap.get(cv2.CAP_PROP_FPS)
    frame_interval = int(fps * interval)  # Number of frames to skip to achieve the desired interval
    ...
    while ret:
        ...
        if cut_right:
            frame1_resized = resize_by_cutting(frame1_resized, percentage=cut_right, side='right')
            frame2_resized = resize_by_cutting(frame2_resized, percentage=cut_right, side='right')
        if cut_left:
            frame1_resized = resize_by_cutting(frame1_resized, percentage=cut_left, side='left')
            frame2_resized = resize_by_cutting(frame2_resized, percentage=cut_left, side='left')
        if cut_bottom:
            frame1_resized = resize_by_cutting(frame1_resized, percentage=cut_bottom, side='bottom')
            frame2_resized = resize_by_cutting(frame2_resized, percentage=cut_bottom, side='bottom')
        ...
        total_movement = sum(cv2.contourArea(c) for c in contours if cv2.contourArea(c) > min_contour_area)
        ...
        if total_movement > movement_threshold:
            if motion_start_str is None:
                ...
                motion_start = frame_count  # Start a new motion period

                if debug:
                    # Create a blank image with the same dimensions as the dilated image
                    contour_img = cv2.cvtColor(dilated, cv2.COLOR_GRAY2BGR)  # Convert to BGR to draw colored contours
                    # Draw the contours on the blank image
                    cv2.drawContours(contour_img, contours, -1, (0, 255, 0), 2)  # Green contours with thickness of 2
...
def resize_by_cutting(frame, percentage=20, side='top'):
    ...
    if side.lower() == 'top':
        ...
        raise ValueError("Invalid side. Choose from 'top', 'bottom', 'left', or 'right'.")
...
for file in video_files:
    detect_motion(file, movement_threshold=1, min_contour_area=1000, interval=4, debug=True, cut_left=35,
              cut_right=15, cut_top=40)